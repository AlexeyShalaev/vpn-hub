...
class Provisioner(Protocol):
    ...
    # Приостановка/возврат доступа клиента БЕЗ потери материала (для лимита трафика — Этап 3b).
    # suspend перекрывает трафик, сохраняя слот/материал; resume возвращает доступ с ТЕМ ЖЕ конфигом,
    # чтобы пользователю не пришлось перевставлять его в клиент. Механизм зависит от протокола.
