...
def disk_total_bytes(path: str) -> int | None:
    """Полный размер тома по пути (для авто-лимита метрик). None — путь недоступен."""
    try:
...
def auto_size_cap_bytes(settings: Any) -> int:
    """Авто-лимит размера метрик = процент от диска (`metrics_disk_cap_pct`% от `metrics_disk_path`).

    0, если авто выключено (pct<=0) или диск недоступен. Используется, когда явный кап не задан.
    """
    pct = int(getattr(settings, "metrics_disk_cap_pct", 0) or 0)
    if pct <= 0:
        return 0
    total = disk_total_bytes(getattr(settings, "metrics_disk_path", "/"))
    return int(total * pct / 100) if total else 0


async def size_cap_bytes(session: Any, settings: Any = None) -> int:
    """Эффективный лимит размера метрик в байтах.

    Приоритет: явный `SETTING_SIZE_CAP_GB` (>0) → авто-процент от диска (`auto_size_cap_bytes`) →
    0 (без лимита). Передайте `settings`, чтобы включить авто-фолбэк.
    """
    ...
    return auto_size_cap_bytes(settings) if settings is not None else 0
...
async def enforce_size_cap(uow: Any, settings: Any) -> dict:
    ...
    async with uow.query() as tx:
        cap = await size_cap_bytes(tx.session, settings)
        if cap <= 0:
